...
class IntermediateRepresentation:
    ...
    def get_ssa_value(self, operand: int | str) -> int:
        if operand in self.ssa_associations:
            ...
            self.basic_blocks[0].append(self.compose_instruction(ssa_value, "const", operand))
            return ssa_value
    ...
    def compose_instruction(self, line_number: int, operation: str, *operands) -> str:
        x, y, *useless = (*operands, "", "")
        operator = self.operation_to_operators[operation]
        return f"{line_number}: {operator} {x} {y}".rstrip()
...
def compile(file_name: str) -> None:
    ...
    def expression() -> int:
        ...
        while file_stream.peek_token() in ["+", "-"]:
            ...
            match token:
                case Token.PLUS:
                    intrepr.compose_instruction(new_ssa_value, Token.PLUS, ssa_value, term())
                    # // FLAG, EMIT INSTRUCTION
                case Token.MINUS:
                    intrepr.compose_instruction(new_ssa_value, Token.MINUS, ssa_value, term())
                    # // FLAG, EMIT INSTRUCTION
    ...
    def term() -> int:
        ...
        while file_stream.peek_token() in ["*", "/"]:
            ...
            match token:
                case Token.MULTIPLY:
                    intrepr.compose_instruction(new_ssa_value, Token.MULTIPLY, ssa_value, factor())
                    # // FLAG, EMIT INSTRUCTION
                case Token.DIVIDE:
                    intrepr.compose_instruction(new_ssa_value, Token.DIVIDE, ssa_value, factor())
                    # // FLAG, EMIT INSTRUCTION
            ssa_value = new_ssa_value
        return ssa_value
